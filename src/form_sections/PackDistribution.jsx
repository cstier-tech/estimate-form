...
// A user-extendable list of pack distributions for ONE completed-units level.
// Each row carries its own pack type plus a { qtyPerPack, numberOfPacks } split,
// e.g. "5000 shrink-wrapped in packs of 10, 2000 banded in packs of 20".
//
// The packed Total Qty is checked against `completedUnits` for this level so an
// over/under pack count is flagged.
function PackDistribution({ rows, onChange, completedUnits = 0, levelIndex = 0 }) {
    const addRow = () => onChange([...rows, createPackDistributionRow(levelIndex)])
    ...
    const target = Number(completedUnits) || 0
        ...
        <div className="space-y-3">
            {rows.map((row, index) => (
                <div key={index} className="flex items-end gap-2">
                    ...
                    />
                    <span className="pb-2 text-sm whitespace-nowrap text-gray-600">= {nf.format(rowTotal(row))}</span>
            ...
            <p className="text-sm font-semibold text-gray-800">Total Qty: {nf.format(totalQty)}</p>
            <p className="text-sm text-gray-700">
                Completed Units: {nf.format(target)}
                {diff === 0
                    ...
                    : <span className={`ml-2 font-semibold ${diff > 0 ? 'text-amber-700' : 'text-red-700'}`}>
                        Total Qty is {nf.format(Math.abs(diff))} {diff > 0 ? 'more' : 'less'}
                    </span>
                }
            </p>
        </div>
